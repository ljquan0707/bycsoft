	...
	if(typeof (s) != "undefined") {
		...
		for(var i = 0; i < sArr.length; i++){
			if(uSinaEmotionsHt.containsKey(sArr[i])) {
				var reStr = "<img src=\"" + uSinaEmotionsHt.get(sArr[i]) + "\" height=\"22\" width=\"22\" vertical-align=\"text-bottom\" />";
	...
	$.fn.SinaEmotion = function(target){
		...
		$(this).click(function(event){
			...
			if($('#emotions .categorys')[0]){
				$('#emotions').css({top: '80px', left:'0px'});
				$('#emotions').toggle();
				return;
			}
			$('.right').append('<div id="emotions"></div>');
			$('#emotions').css({top: '188px', left:'0px'});
